<?php
include('../config/session.php'); // Checks login
...
if(isset($_GET['remove_id'])) {
    ...
    // 1. Fetch the token first (to check if it's the current device)
    ...
    if(mysqli_num_rows($check_res) > 0) {
        ...
        if(mysqli_query($conn, $del_sql)) {
            ...
            if(isset($_COOKIE['remember_me']) && $_COOKIE['remember_me'] === $token_to_remove) {
                // Destroy the cookie immediately so this browser is no longer trusted
